...
def test_arg_parsing_update_command(cli_mocker, tmp_path, prefix_args, args, mods):
    ...
    mock = cli_mocker(main_func="binstar_client.commands.update.main")

    if "--no-release" in args and mock.parser == "original":
        return  # Skip because this option isn't handled by argparse
...
def test_arg_parsing_groups_command(cli_mocker, prefix_args, args, mods):
    spec = "my-org/my-group/my-member"

    mock = cli_mocker(main_func="binstar_client.commands.groups.main")
    result = mock.invoke(["groups", *args, spec], prefix_args=prefix_args)
    assert result.exit_code == 0, result.stdout
    mock.assert_main_called_once()
...
def test_arg_parsing_show_command(cli_mocker, prefix_args, args, mods):
    spec = "someone/some-package/some-version"

    mock = cli_mocker(main_func="binstar_client.commands.show.main")
    result = mock.invoke(["show", *args, spec], prefix_args=prefix_args)
    assert result.exit_code == 0, result.stdout
    mock.assert_main_called_once()
